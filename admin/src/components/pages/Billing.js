...
import Blngdtl from "../templates/bookingdtl/Blngdtl";
...
const Billing = () => {
  ...
  const billing = async (bypass) => {
    ...
    if (res.status !== 200) {
      setprcs(false);
      return alert("failed to load the Billing");
    } else {
      setlst({
        ...lst,
        prv: { pag, entry },
  ...
  return (
    <>
      <Helmet>
        <meta charSet="utf-8" />
        <title>Billing - RevaCabs</title>
        <meta name="description" content="Admin Billing managment " />
      ...
      {prcs ? (
        ...
        <Blngdtl dtl={dtl} setdtl={setdtl} billing={billing} />
      ) : (
