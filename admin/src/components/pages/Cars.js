...
import Cardtl from "../templates/oprtrdtl/Cardtl";
import { Helmet } from "react-helmet";
const Cars = () => {
  const df = { oprtrid: "", rcnum: "", status: "" };
  const [lst, setlst] = useState({ entry: "10", pag: 1, data: [] });
  const [filter, setfilter] = useState(df);
  const [dtl, setdtl] = useState({ display: false, rcnum: "" });
  ...
  const cars = async () => {
    const { oprtrid, rcnum, status } = filter;
    let { entry, pag } = lst;
  ...
  // === === === list loader end here === === === //

  const intator = () => {
    if (lst.flt) {
      const { oprtrid, rcnum, status } = lst.flt;
      if (
  ...
  return (
    <>
      <Helmet>
        <meta charSet="utf-8" />
        <title>Cars - RevaCabs </title>
        <meta name="description" content="Manage Cars" />
