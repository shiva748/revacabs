  ...
  try {
    ...
    const user = await Partner.findOne({
      ...
      .catch((error) => {
        return res
          .status(400)
          .json({ error: "failed to fetch data from server" });
      });
    if (!user) {
...
exports.partner_autolog = (req, res) => {
  ...
  let data = {
    ...
    reqvrftn: req.user.verification.request,
  };
...
exports.partner_profilepic = async (req, res) => {
  const options = {
    root: path.join(__dirname, "../public/privateimages/profiles/"),
  };

  var fileName = req.user.Profile ? req.user.Profile : "pic.png";
  if (
    !fs.existsSync(
      path.join(__dirname, "../public/privateimages/profiles/", fileName)
    )
  ) {
    fileName = "pic.png";
  }
...
exports.partner_verify = async (req, res) => {
  ...
  if (user.faultin.dl) {
    ...
    const dlupload = uploadimage(dlimg, dln, "../public/privateimages/dls/");
    if (!dlupload.result) {
  ...
  const update = await Partner.updateOne(
    ...
    .catch((error) => {
      return res.status(500).json("failed to update data");
    });
  if (update) {
    if (!user.operatorid) {
...
exports.partner_addriver = async (req, res) => {
  ...
  const isep = await Driver.findOne(
    ...
    .catch((error) => {
      return res.status(500).json("failed to fetch data from server");
    });
  if (isep) {
    if (isep.email === email) {
...
exports.partner_updatedriver = async (req, res) => {
  ...
  if (driver.verification.isverified) {
    ...
    const update = await Driver.updateOne({ driverid }, { status })
      ...
      .catch((error) => {
        return res.status(500).json("falied to fetch data");
      });
    if (update) {
...
exports.partner_addcar = async (req, res) => {
  ...
  const vhclupload = uploadimage(
    vehicleimg,
    vhcln,
    "../public/privateimages/vehicles/"
  );
  if (!vhclupload.result) {
    return res.status(400).json(vhclupload.message);
  }
  const rcupload = uploadimage(rcimg, rcn, "../public/privateimages/rcs/");
  if (!rcupload.result) {
  ...
  if (cab.verification.isverified) {
    ...
    if (cab.faultin.permit) {
      ...
      if (
        ...
        typeof permitimg !== "string" ||
        typeof permitValidity !== "number" ||
        !["State", "National"].some((itm) => permitType === itm)
      ) {
        return res.status("invalid data type");
      }
      const permitupload = uploadimage(
        data.permitimg,
        cab.permitLink,
    ...
    const update = await Cars.updateOne({ cabid }, ud)
      ...
      .catch((error) => {
        return res.satus(500).json("failed to fetch data");
      });
    if (update) {
...
exports.partner_bid = async (req, res) => {
  ...
  if (booking.tripinfo.equivalent.isequi) {
    car = await Cars.findOne({
      operatedby: user.operatorid,
      group_id: { $gte: booking.tripinfo.group_id },
    ...
    car = await Cars.findOne({
      ...
      $or: [
        ...
        { group_id: { $gt: booking.tripinfo.group_id } },
      ],
      "verification.isverified": true,
    })
      .then((res) => {
...
exports.partner_triplog = async (req, res) => {
  ...
  if (log) {
    return res.json({ result: "success", ary: log });
  } else {
...
exports.partner_assigndc = async (req, res) => {
  ...
  if (booking.cabinfo.cabid !== car) {
    ...
    if (booking.tripinfo.equivalent.isequi) {
      if (cab.group_id < booking.tripinfo.group_id) {
        return res
          .status(400)
          .json(
...
exports.driver_submitkm = async (req, res) => {
  ...
  const isbooking = await Booking.findOne({
    ...
    .catch((error) => {
      return res.status("failed to fetch data");
    });
  if (!isbooking) {
  ...
  const result = await Booking.updateOne(
    ...
    .catch((error) => {
      return res.status(500).json("failed to fetch data from");
    });
  if (!result) {
    ...
    return res
      .status(201)
      .json(
        "an otp has been sent to coustumer and it's also visible on coustumers dashboard"
      );
    ...
    //   })
    //   .catch((error) => {
    //     return res.status("failed to send otp to client");
    //   });
...
exports.driver_verandtrip = async (req, res) => {
  ...
  if (!isbooking) {
    return res.status(400).json("No booking found 1");
  }
