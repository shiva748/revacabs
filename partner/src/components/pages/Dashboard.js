...
import { FaRupeeSign, FaTimes, FaSearch } from "react-icons/fa";
...
const Dashboard = () => {
  ...
  const reqbooking = async (bypass) => {
    ...
    setldng(true);
    let res = await fetch("/partner/booking", {
      method: "POST",
      headers: {
  ...
  const postbid = async () => {
    ...
    setbdata({...bdata, load:true})
    let res = await fetch("/partner/bid", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        amt: bdata.amt,
        bookingid: bdata.data.bookingid,
      }),
    });
    let data = await res.json();
    if (res.status !== 200) {
      alert(data);
      setbdata({...bdata, load:false});
    } else {
      reqbooking(true);
      setbdata({ display: false, data: "", amt: "", load:false });
      alert(data);
    }
  };
                ...
                return (
                  <div className="bkng-crd" key={i}>
                    ...
                    <div className="gv-con">
                      {itm.bids.some((e) => e.operatorid === id) ? (
                        <button
                          className="gv-btn"
                          onClick={() => {
                            setbdata({
      ...
      {bdata.display ? (
        <div className="form-container">
          <div className="bd-con">
            <div className="bkng-crd-hd" style={{ justifyContent: "left" }}>
              {bdata.data.bookingid}{" "}
              <div
                className="clsr-con"
                onClick={() => {
                  setbdata({ display: false, data: "", amt: "" });
                }}
              >
                <FaTimes />
              </div>
            </div>
            <div className="form-row bold-txt" style={{ width: "100%" }}>
              {bdata.data.sourcecity.from.split(",")[0]}
              {bdata.data.bookingtype === "Local" ? (
                ""
              ) : bdata.data.outstation === "Roundtrip" ? (
                <>
                  <CgArrowsExchange />
                  {bdata.data.endcity.to.split(",")[0]}
                </>
              ) : (
                <>
                  <CgArrowRight />
              ...
              {bdata.data.bookingtype === "Local"
                ? `(${bdata.data.bookingtype} ${bdata.data.tripinfo.hour}hr - ${bdata.data.tripinfo.distance}km)`
                : `(${bdata.data.outstation})`}
            </div>
            <div className="form-row">
              <div className="irt" style={{ backgroundColor: "#0d0c3a" }}>
                <FaRupeeSign />
              </div>
              <input
                type="number"
                ...
                onChange={(e) => {
                  // eslint-disable-next-line
                  setbdata({ ...bdata, ["amt"]: e.target.value });
                }}
              />
            </div>
            <div className="form-row">
              {bdata.action === "nwbd" ? (
                <button
                  className={bdata.load?"gv-btn load-btn":"gv-btn"}
                  style={{ margin: "auto" }}
                  onClick={postbid}
                >
                  <span>Submit</span>
